...
#define UDPPortA 21834   // ServerA Port number
...
void UDPServer::setup()
{
    ...
    soc_server.sin_port = htons(UDPPortA);
...
void UDPServer::sendtoaws(double *bf)
{
	...
	if (s1 == -1)
	    {
            cout<<"Error while sending data from Server A to AWS!!!"<< endl;
            exit(0);
	    }
	else
	{
    	cout<<"The Server A has finished sending the output to AWS"<<endl;
...
double *File::search(double l)
{
    ...
    cout<<"The Server A has found "<<flag<<" match"<<endl;
    return arr;
}
...
//    file.displayfile();
    ...
    {     
        ...
        cout<<"The Server A received input <"<<mes<<">"<<endl;  
